...
def big_seed
  ...
  psy_array.each do |hash|
    ...
    puts "#{count} psys créés"
    user.save!
  end
  puts "#{Office.count} offices créées"
  puts "#{User.count} utilisateurs crées"
  puts "#{Job.count} jobs crées"
end
...
puts "voulez-vous lancer le petit ou le gros seed ?"
puts "1 - petit seed"
puts "2 - GROS SEED"
